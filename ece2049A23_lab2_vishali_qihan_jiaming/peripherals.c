...
unsigned int getButton(void){
    unsigned int ret_val = 0;
        ...
        if ((P3IN & BIT6)==0) //S2
            ret_val = 0x02;
        if ((P7IN & BIT0)==0 && (P3IN & BIT6)==0) // S1 S2
            ret_val = 0x03;
        if ((P2IN & BIT2)==0) //S3
            ret_val = 0x04;
        if ((P2IN & BIT2)==0 && (P7IN & BIT0)==0) //S3 S1
            ret_val = 0x05;
        if ((P2IN & BIT2)==0 && (P3IN & BIT6)==0) // S3 S2
            ret_val = 0x06;
        if ((P2IN & BIT2)==0 && (P3IN & BIT6)==0 && (P7IN & BIT0)==0) //S3 S2 S1
            ret_val = 0x07;
        if ((P7IN & BIT4)==0) //S4
            ret_val = 0x08;
        if ((P7IN & BIT4)==0 && (P7IN & BIT0)==0) //S4 S1
            ret_val = 0x09;
        if ((P2IN & BIT2)==0 && (P3IN & BIT6)==0) //S4 S2
            ret_val = 0x0A;
        if ((P2IN & BIT2)==0 && (P3IN & BIT6)==0 && (P7IN & BIT0)==0) //S4 S2 S1
            ret_val = 0x0B;
        if ((P1IN & BIT4)==0 && (P2IN & BIT2)==0)// S4 S3
            ret_val = 0x0C;
        if ((P2IN & BIT2)==0 && (P3IN & BIT6)==0 && (P7IN & BIT0)==0) //S4 S3 S1
            ret_val = 0x0D;
        if ((P2IN & BIT2)==0 && (P2IN & BIT2)==0 && (P3IN & BIT6)==0) // S4 S3 S2
            ret_val = 0x0E;
        if ((P1IN & BIT4)==0 && (P2IN & BIT2)==0 && (P3IN & BIT6)==0 && (P7IN & BIT0)==0) //S4 S3 S2 S1
            ret_val = 0x0F;
